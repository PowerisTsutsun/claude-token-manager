...
const POLL_INTERVAL_MS = 2000; // fallback poll interval
 ...
 */
function findProjectJsonlDir(workspacePath) {
...
function mostRecentJsonlFile(dir) {
...
function computeSessionStats(entries) {
    ...
    if (totalCost === 0) {
        totalCost = (totalInput / 1000000) * SONNET_IN_PRICE_PM
...
// ─────────────────────────────────────────────────────────────────────────────
function callAnthropicApi(apiKey, model, system, user) {
...
class MemorySystem {
    constructor(_ctx, workspaceRoot) {
    ...
    get exists() { return fs.existsSync(this.memDir); }
    get memoryFilePath() { return this.memoryFile; }
    get bugFixFilePath() { return this.bugFixFile; }
    initialize() {
    ...
    async summarizeSession(entries, apiKey) {
        ...
        try {
            ...
            vscode.window.showErrorMessage(`Claude Token Manager: Summarization failed — ${e.message}`);
            return;
        }
        let summary;
        try {
            ...
            vscode.window.showErrorMessage('Claude Token Manager: Could not parse summarization response.');
            return;
        }
        this.mergeSummary(summary);
        vscode.window.showInformationMessage('Claude Token Manager: Session summarized and memory updated.');
    }
    ...
    async loadToClipboard() {
        const memory = this.readMemory();
        if (!memory || memory.includes('No sessions summarized yet')) {
            return;
        }
        const tokens = this.estimateTokens(memory);
        await vscode.env.clipboard.writeText(memory);
        vscode.window.showInformationMessage(`Claude memory loaded (~${tokens} tokens) — paste at the start of your prompt with Ctrl+V to give Claude full project context`);
    }
    parseBugFixes() {
        try {
...
class TokenTracker {
    ...
    startWatching() {
        ...
        try {
            this.dirWatcher = fs.watch(this.projectDir, { persistent: false }, () => this.refresh(false));
        }
    ...
    refresh(initialLoad) {
        ...
        this.updateStatusBar();
        const ids = new Set(entries.map(e => e.sessionId).filter((id) => !!id));
    ...
    resetIdleTimer(entries) {
        if (this.idleTimer) {
            clearTimeout(this.idleTimer);
        }
        this.idleTimer = setTimeout(() => {
            if (this.onSessionEnd && entries.length) {
                this.onSessionEnd(entries);
            }
        }, SESSION_IDLE_MS);
    }
    updateStatusBar() {
...
// ─────────────────────────────────────────────────────────────────────────────
function getTokenPanelHtml(session, nonce) {
...
// ─────────────────────────────────────────────────────────────────────────────
function getBugFixDetailHtml(content, nonce) {
...
async function activate(ctx) {
    ...
    if (workspaceRoot) {
        ...
        tracker.onNewSession = async () => {
            if (memorySys) {
                await memorySys.loadToClipboard();
            }
        };
    ...
    if (!hasKey) {
        vscode.window.showInformationMessage('Claude Token Manager needs your Anthropic API key for auto-summarization.', 'Add API Key').then(sel => {
            if (sel) {
                vscode.commands.executeCommand('claude-token-manager.setApiKey');
            }
        });
    }
    // ── Commands ───────────────────────────────────────────────────────────────
    // Show / reveal the token usage webview panel
    ctx.subscriptions.push(vscode.commands.registerCommand('claude-token-manager.showTokenPanel', () => {
        if (tokenPanel) {
            tokenPanel.reveal();
            return;
        }
        tokenPanel = vscode.window.createWebviewPanel('claudeTokenUsage', 'Claude Token Usage', vscode.ViewColumn.Beside, { enableScripts: true, retainContextWhenHidden: true });
        tokenPanel.webview.html = getTokenPanelHtml(tracker?.getSession() ?? null, generateNonce());
    ...
    ctx.subscriptions.push(vscode.commands.registerCommand('claude-token-manager.setApiKey', async () => {
    ...
    ctx.subscriptions.push(vscode.commands.registerCommand('claude-token-manager.summarizeNow', async () => {
        const key = await ctx.secrets.get(SECRET_KEY_ID);
        if (!key) {
            vscode.window.showErrorMessage('Claude Token Manager: No API key set. Run "Claude Token Manager: Set API Key" first.');
            return;
        }
        const session = tracker?.getSession();
        if (!session?.rawEntries.length || !memorySys) {
            vscode.window.showWarningMessage('Claude Token Manager: No session data available to summarize.');
            return;
        }
    ...
    // Open MEMORY.md
    ctx.subscriptions.push(vscode.commands.registerCommand('claude-token-manager.openMemory', async () => {
        if (!memorySys) {
            return;
        }
    ...
    // Open BUG_FIXES.md
    ctx.subscriptions.push(vscode.commands.registerCommand('claude-token-manager.openBugFixes', async () => {
        if (!memorySys) {
            return;
        }
    ...
    ctx.subscriptions.push(vscode.commands.registerCommand('claude-token-manager.searchBugFixes', async () => {
        if (!memorySys) {
            return;
        }
        const items = memorySys.parseBugFixes();
        if (!items.length) {
            vscode.window.showInformationMessage('Claude Token Manager: No bug fixes recorded yet.');
            return;
        }
        const picked = await vscode.window.showQuickPick(items.map(i => ({ ...i, alwaysShow: true })), {
            placeHolder: 'Search bug fixes by title, tag, or file…',
            matchOnDescription: true,
            matchOnDetail: true,
        });
        if (picked) {
            const panel = vscode.window.createWebviewPanel('bugFixDetail', `Bug: ${picked.label}`, vscode.ViewColumn.Beside, { enableScripts: false });
            panel.webview.html = getBugFixDetailHtml(picked.full, generateNonce());
        }
    }));
    // Clear all memory files and start fresh
    ctx.subscriptions.push(vscode.commands.registerCommand('claude-token-manager.clearMemory', async () => {
        const confirm = await vscode.window.showWarningMessage('Clear all Claude Token Manager memory files? This cannot be undone.', { modal: true }, 'Clear Memory');
        if (confirm === 'Clear Memory' && memorySys) {
            memorySys.clearAll();
            vscode.window.showInformationMessage('Claude Token Manager: Memory cleared and reset.');
        }
    }));
}
// ─────────────────────────────────────────────────────────────────────────────
